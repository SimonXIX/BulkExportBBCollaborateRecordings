...
class PanoptoUploader:
    def __init__(self, server, ssl_verify, oauth2, username, password): # mbk added username pass for user auth.
        '''
        ...
        # self.__setup_or_refresh_access_token() # comment out to elimitate 3LO and browser
        self.__setup_resource_owner_grant_access_token()

    def __setup_or_refresh_access_token(self):  # Used for 3LO flow... we're going to just get a token with 2LO
        '''
    ...
    def __setup_resource_owner_grant_access_token(self):  # Used for 3LO flow... we're going to just get a token with 2LO
        '''
    ...
    def upload_video(self, file_path, folder_id):
        ...
        url = self.__monitor_progress(upload_id)
        return url
